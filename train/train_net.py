...
def load_synthtext(image_set, devkit_path, shuffle=False):
    """
    wrapper function for loading pascal voc dataset
    ...
    """
    print("loading data...")
    Imdb = SynthText(image_set, devkit_path, shuffle, is_train=True)
...
def train_net(net, dataset, image_set, devkit_path, batch_size,
    ...
    # load dataset
    if dataset == 'icdar':
        imdb = load_icdar(image_set, devkit_path, cfg.TRAIN.INIT_SHUFFLE)
        if val_set:
            val_imdb = load_icdar(val_set, devkit_path, False)
        else:
            val_imdb = None
    elif dataset == 'SynthText':
        imdb = load_synthtext(image_set, devkit_path, cfg.TRAIN.INIT_SHUFFLE)
        if val_set:
            val_imdb = load_synthtext(val_set, devkit_path, False)
        else:
            val_imdb = None
    else:
